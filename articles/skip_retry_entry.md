...
Liquid Glass時代にはタブは`Tab`を、タイトルは`inlineLargeTitle`を採用したいところです。対応しているかわかりませんが`ContentView.swift`の中身を編集してみます。

元々プロジェクトのminimumがiOS 17に設定されていたのでこれを変更して進めます。Xcodeのプロジェクト設定を切り替えるのはもちろんのこと、Skip.envファイルの下にあるAporoidTodoというファイルにもターゲット周りの設定が書いてあるので二箇所を修正しました。
